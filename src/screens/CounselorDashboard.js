...
const CounselorDashboard = ({ navigation }) => {
  ...
  const myRequests = useMemo(() => {
    ...
    return state.leaveRequests
      .filter((r) => r.counselorId === user.id)
      .sort(sortByCreatedAtDesc);
  }, [state.leaveRequests, user]);

  const filtered = useMemo(() => {
    if (tab === 'pending') return myRequests.filter((r) => r.counselorStatus === 'Pending');
    if (tab === 'recommended') return myRequests.filter((r) => r.counselorStatus === 'Recommended');
    if (tab === 'rejected') return myRequests.filter((r) => r.counselorStatus === 'Rejected');
    return myRequests;
  }, [myRequests, tab]);
